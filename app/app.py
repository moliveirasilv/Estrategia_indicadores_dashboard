...
from dash_table.Format import Format, Group, Prefix, Scheme, Symbol # Localization
...
tblcols_3 = [{'name':'Código','id':'Código'},
            #  {'name':'Último Fechamento vs Max 52','id':'Fechamento vs|Máx 52 sem||em %','type':'numeric','format':formated_percentage},
             {'name':'Max 52','id':'Máximo do|Fechamento|52 semanas|em R$','type':'numeric','format':formated_price},
             {'name':'Retorno 1d','id':'Retorno|1 dia||em %','type':'numeric','format':formated_percentage},
             {'name':'Retorno 1 mês','id':'Retorno|1 mês||em %','type':'numeric','format':formated_percentage},
             {'name':'Retorno 12 meses','id':'Retorno|12 meses||em %','type':'numeric','format':formated_percentage},
             {'name':'Volatilidade','id':'Volatilidade|12 meses||em %','type':'numeric','format':formated_percentage},
...
tblcols_4 = [{'name':'Call 1','id':'Call 1'},
             ...
             {'name':'Ultimo Preço(Call 1)','id':'Ultimo Preço(Call 1)','type':'numeric','format':formated_price},
             {'name':'Ultimo Preço(Call 2)','id':'Ultimo Preço(Call 2)','type':'numeric','format':formated_price},
...
app = dash.Dash(__name__, external_stylesheets =[dbc.themes.FLATLY]) ## start app
    ...
    dcc.Tabs([
        ...
        dcc.Tab(label='Indicadores Financeiros', children=[
            ...
            dash_table.DataTable(id='table-retornos',
                                 columns=tblcols_3,
                                 data=  comparaveis[comparaveis['Código'] =='PETR4'].to_dict('records'),
                                 sort_action="native",
                                 sort_mode='multi',
                                 page_action='native',
                                 style_cell = {'font-family': fontfamily,'fontSize': fontsize}
...
def updateRetornos(input_ativo):
    return comparaveis[comparaveis['Código'] == '{}'.format(input_ativo)].to_dict('records')

@app.callback(Output('table-retornos-comparaveis','data'),
              Input('empresas_dropdown','value'))

def updateRetornosComparaveis(input_ativo):
    setores = str(*comparaveis['Subsetor Bovespa'][comparaveis['Código'] == '{}'.format(input_ativo)])
    return comparaveis[(~comparaveis['Código'].str.contains('{}'.format(str(input_ativo).rstrip('123456789')))) & (comparaveis['Subsetor Bovespa'] == setores)].to_dict("records")
